...
VRM 3D desktop pets on a transparent macOS overlay (Electron + three.js + [@pixiv/three-vrm](https://github.com/pixiv/three-vrm)) — but they're not just decoration. **Each pet is bound to a real coding agent** (OpenAI Codex or Claude Code) and a working directory: say the word in its speech bubble and your pet goes off to edit code, run tests, and write docs — asking for your approval before anything risky, then reporting back with expressions and motions.
...
- **No API bills** — everything runs through locally installed CLIs with subscription login (Codex CLI / Claude Code). No API keys, no surprise invoices.
...
- **Desktop-pet polish** — pixel-level click-through (your mouse passes straight through transparent areas), gaze tracking, drag/rotate/zoom, random idle motions; **per-part physics tuning**: hair, clothes, and tail sway are each adjustable — that fluffy fox-tail bounce is something you dial in yourself. Idle throttling, battery/thermal downshifting, and full stop on screen lock keep it light as a background app.
...
- Lighting (directional/point light, shadows), **per-part physics sway** (hair / clothes / chest / tail — independent spring-bone tuning), outfit visibility toggles
...
- Each pet binds to **Codex** (`codex app-server`, persistent JSON-RPC) or **Claude Code** (CLI spawn); dynamic model list, reasoning effort low–ultra, persona injected into the system prompt
- **Chat queue**: the input box never locks while a turn is running; messages queue up and continue automatically, each removable
...
- **Bubble niceties**: pin to keep it open, drag the side edges to set width (double-click to reset), width adapts to content, and the cap tightens automatically when many pets are awake
...
- Node.js 22+
- For AI features: [Codex CLI](https://github.com/openai/codex) and/or [Claude Code](https://claude.com/claude-code) installed and logged in (subscription account)
