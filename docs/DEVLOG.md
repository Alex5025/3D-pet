...
## 57. 介面重設計⑦:右鍵選單精簡兩層(2026-08-27)

**動機**:§55 的自繪選單結構照搬原生(15 項+5 條分隔線),使用者反映醜且複雜。

**重整**:主選單只留天天用的四項(播放動作/設定/中控面板/休息-喚醒)+「更多」;其餘九項(預設姿勢/選 VRM/切換寵物/重置位置/重啟此寵/沙盒/新增寵物/重啟系統/結束)收進「更多」子頁,危險動作固定底部、結束維持紅色。「設定」砍掉四分頁子頁直接開設定視窗(視窗裡本來就有同樣分頁,少一層重複導航);「停止動作」移進播放動作子頁首項。子頁返回鏈修正:預設姿勢/切換寵物由「更多」進入,返回也回「更多」。視覺:每項前置 emoji 圖示(固定 20px 插槽對齊,語言中立不進 i18n);標頭放寵物大頭照——右鍵當下 `viewer.snapshot(false)` 即時截一張(與 sendAvatarIcons 同款),截不到退回狀態點。i18n 僅新增 menu.more。

**驗證**:menutest 互動回歸——主選單恰 5 個可點項、「設定」記錄 openSettings('light') 且關閉、更多→預設姿勢→返回落在更多頁、播放動作頁首項為停止動作、單寵情境(stub 改單寵)更多頁不出現切換寵物、點外收合;深淺截圖 OK。
