...
**桌寵本體**
- 任意替換 VRM 模型(Tray 選單 / 拖放到寵物接收窗),多寵物同時運行,各自獨立設定
- 視線跟隨游標、拖曳移動、右鍵旋轉、滾輪縮放,透明區域點擊穿透(像素級命中)
- 光影調整(平行光/點光源、位置墊、陰影濃度)、逐部位晃動強度(頭髮/衣服/胸部/尾巴)、服裝顯示開關
- VRMA 動作播放、開機預設姿勢、**待機動作**(勾選一組動作,20~60 秒隨機自動播放)
- **功率檔位**:閒置自動節流 + `powerMonitor` 聯動——電池/過熱自動降輪詢與幀率,鎖屏/睡眠全停
...
- 審批可在拒絕時附上調整方向,讓 Codex / Claude 在同一輪依回饋修正做法
- session 持久化:重啟 app 繼續上次對話;可中斷、crash 自動重連;泡泡「＋ 新對話」一鍵清空上下文
- 回覆 Markdown 渲染(GFM,DOMPurify 消毒)、模型/力度徽章、外側執行狀態燈號(已讀後自動隱藏)
- **圖片訊息**:可把 PNG/JPEG/WebP 直接貼進輸入框(最多 4 張、單張 8 MiB),泡泡不產生預覽但會把圖片交給 AI 閱讀;也支援純圖片訊息
- **泡泡圖釘**:滑鼠靠近遠離螢幕中心的上方角落才顯示;釘選後長期展開。滑鼠在泡泡出現時按住,即使移出寵物也會保持到放開
- **拖放參考檔案**:把文件/資料夾拖向寵物,亮出的接收窗放開即加入——絕對路徑列於泡泡最下方並注入對話,AI 知道去哪查找/更新(當次對話有效,✕ 可移除)
- **寵物工具(MCP)**:agent 可自主呼叫 `pet_play_motion` / `pet_show_expression` / `pet_speak` 配合情緒表演

**專案沙盒與安全**
- 右鍵選單的「沙盒設定…」是獨立高風險視窗,不會出現在一般設定分頁;由 Electron main 直接管理工作目錄的 `.codex/config.toml`,不經 AI、不接受任意腳本或 shell 字串
- 可設定 Codex 的核准策略、沙盒模式與 workspace-write 網路存取;完整存取會二次確認,設定需在新的 Codex 工作階段生效
- `workspace-write` 通常仍會保護 `.git`;要執行 commit/tag/push,需使用允許核准的工作權限並在提示時明確同意,或在可信專案中短暫啟用完整存取
...
- **右鍵角色**:依工作目錄切換寵物、選 VRM 檔、角色動作(播放/預設姿勢/停止)、一般設定、獨立沙盒設定、重置位置、休息/喚醒、重新啟動目前寵物或整個寵物系統。
- **設定面板**(右鍵 → 設定 → 各分頁):
  - 光影:光源類型/強度/位置、陰影濃度
  - 角色:寵物名稱、休息/喚醒、個性(persona)、晃動強度、服裝、位置
  - 動作:預設姿勢、待機動作勾選清單
  - 工作:AI 助手(Codex/Claude)、模型、推理力度、權限、Session、工作目錄
- 對話:滑到角色上 → 泡泡輸入 → Enter 送出(Shift+Enter 換行);拖檔案給寵物即成參考檔案。
- 圖片:複製圖片後在泡泡輸入框貼上;只顯示附件數量,送出後由 AI 讀取。

模型放 `models/`、動作(.vrma)放 `motions/`;運行資料在 `runtime-data/`(皆不入版控)。

> `.codex/config.toml` 可能包含專案沙盒策略與本機路徑。提交前請確認內容適合分享;本機憑證、token、API key 與帳密不得寫入 repository。
